        ...
        try:
            ...
            if 'username' not in session or '@' in session.get('username', ''):
                try:
                    ...
                    print(f"Gagal ambil username dari 'profiles' (fallback): {e}")
...
<body class="min-h-screen flex flex-col">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const flashMessages = document.querySelectorAll('[class*="bg-"] .animate-fade-in');
    ...
    try:
        ...
        <div class="text-center py-12">
            <div class="bg-white rounded-2xl p-8 max-w-2xl mx-auto">
                ...
                <h2 class="text-2xl font-bold text-gray-900 mb-2">Sementara Tidak Tersedia</h2>
                <p class="text-gray-600 mb-6">Dashboard sedang dalam perbaikan. Silakan navigasi ke menu lain.</p>
                <div class="flex flex-wrap gap-4 justify-center">
                    <a href="{{ url_for('pemasukan_page') }}" class="btn-consistent btn-primary">
                        <i class="fas fa-arrow-down mr-2"></i>Pemasukan
                    </a>
                    <a href="{{ url_for('pengeluaran_page') }}" class="btn-consistent btn-primary">
                        <i class="fas fa-arrow-up mr-2"></i>Pengeluaran
                    </a>
                    <a href="{{ url_for('laporan_page') }}" class="btn-consistent btn-primary">
...
def laporan_page():
    ...
    def filter_by_date(df, start_date, end_date, date_column='Tanggal'):
        if df.empty or date_column not in df.columns: 
            return pd.DataFrame()
        try:
            # Pastikan kolom Tanggal sudah diolah dan dibersihkan dari timezone
    ...
    try:
        ...
        # Jurnal Periode (untuk Laba Rugi dan Jurnal Umum)
        jurnal_df_f = filter_by_date(jurnal_df, mulai_dt, akhir_dt)
        
        # Jurnal Total (untuk Neraca dan Buku Besar)
        ...
        # PASTIKAN jurnalf_df_f sudah berisi data yang benar-benar dalam periode
        if not jurnal_df_f.empty:
            ...
            # KUNCI PERBAIKAN: Nilai laba bersih sudah benar, pastikan dikonversi float
        ...
        if not jurnal_total.empty:
            ...
            # TOTAL PASIVA = Kewajiban + (Modal Murni + Historical Balancing + Laba Bersih)
        ...
        if not jurnal_df.empty and 'YearMonth' in jurnal_df.columns:
            try:
                # ... (Logika Chart Data tetap sama, lewati untuk brevity) ...
                pass
...
def handle_error(e):
    # KUNCI PERBAIKAN: Mengalihkan ke halaman Login
    print(f"❌ [FATAL ERROR] {str(e)}")
    print(traceback.format_exc())
    
    flash(f"Terjadi error sistem: {str(e)}", "danger")
    return redirect(url_for('login_page'))
